...
Eigen::MatrixXd dynamics(Eigen::MatrixXd sigma_state)
{
    ...
    for(int i=0; i < x_sigmavector_size; i++)
    {
        ...
        //ROS_INFO("x3 predict: %.3f", sigma_state(member_x3, i) );
        ...
        //ROS_INFO("vq: x:%.3f y:%.3f z:%.3f",vq_gnd_(0),vq_gnd_(1),vq_gnd_(2));
...
void predict()
{
    ...
    //cout << "x_sigmavector: " << x_size << endl;
    //cout << "x_sigmavector: " << endl << x_sigmavector << endl;

    x_sigmavector_ = dynamics(x_sigmavector);
    //cout << "x_sigmavector_: " << endl << x_sigmavector_ << endl;
    ...
    //cout << "y_sigmavector: " << y_sigmavector << endl;
    ...
    for(int i = 0; i < x_sigmavector_size; i++)
    {
        ...
        //cout << "x3: " << x_sigmavector.col(i)(2) << endl;
        //cout << y_hat_ << endl << " += " << w_m(i) << " * " << y_sigmavector_.col(i) << endl << endl;
    }
...
Eigen::MatrixXd state_to_measurement(Eigen::MatrixXd sigma_state)
{
    ...
    for(int i = 0; i < x_sigmavector_size; i++)
    {
        ...
        //cout << "predict x3: " << sigma_state(member_x3, i) << endl;
        ...
        predict_sigma_measure(member_mdc, i) =  1 / sigma_state(member_x3, i);
    ...
    //ROS_INFO("mdc:%.3f", predict_sigma_measure(member_mdc, 0));
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        ...
        measure_vector << cam1.u, cam1.v, dyn1.depth_cal, dyn1.cam_posegnd(0), dyn1.cam_posegnd(1), dyn1.cam_posegnd(2);
...
*/
        ...
        // error
        cout << "Depth error: " << ( 1/x(2) - dyn1.carcam_pose(2) ) << endl <<
            "Depth: " << 1/x(2) << endl <<
            "Position error: " << endl <<
            "x (m)   " << ( x(3) - dyn1.car_posegnd(0) ) << endl <<
            "y (m)   " << ( x(4) - dyn1.car_posegnd(1) ) << endl <<
            "z (m)   " << ( x(5) - dyn1.car_posegnd(2) ) << endl <<
            "Velocity error: " << endl <<
            "x (m/s) " << ( x(6) - dyn1.car_velgnd(0) ) << endl <<
            "y (m/s) " << ( x(7) - dyn1.car_velgnd(1) ) << endl <<
            "z (m/s) " << ( x(8) - dyn1.car_velgnd(2) ) << endl << endl;


        gt_value.feature_1.data = (cam1.u - cam1.cu) / cam1.fx;
        gt_value.feature_2.data = (cam1.v - cam1.cv) / cam1.fy;
        gt_value.feature_3.data = 1 / dyn1.carcam_pose(2);
        gt_value.feature_4.data = dyn1.carcam_pose(2);
        ...
        estimate_value.feature_4.data = 1 / x(2);
