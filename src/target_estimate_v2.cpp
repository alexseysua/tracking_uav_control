...
Eigen::MatrixXd state_to_measurement(Eigen::MatrixXd sigma_state)
{
    ...
    for(int i = 0; i < x_sigmavector_size; i++)
    {
        ...
        predict_sigma_measure(member_md, i) = 1 / sigma_state(member_x3, i);
...
int main(int argc, char **argv)
{
    ...
    //cout << "process_noise_8: " << process_noise(8, 8) << endl;
    ...
    while(ros::ok())
    {
        ...
        measure_vector << cam1.u, cam1.v, dyn1.depth, dyn1.cam_posegnd(0), dyn1.cam_posegnd(1), dyn1.cam_posegnd(2);
...
*/
        ...
        //cout << "Depth error: " << 1 / x(2) - dyn1.depth << endl <<
        //    "Depth: " << 1 / x(2) << endl;
 
        cout << "states error: " << endl <<
            "x1: " << ( x(0)- ((cam1.u - cam1.cu) / cam1.fx) ) / ((cam1.u - cam1.cu) / cam1.fx) * 100 << endl <<
            "x2: " << ( x(1)- ((cam1.v - cam1.cv) / cam1.fy) ) / ((cam1.v - cam1.cv) / cam1.fy) * 100 << endl <<
            "x3: " << ( x(2)- (1 / dyn1.depth) ) / (1 / dyn1.depth) * 100 << endl;
        ...
        gt_value.feature_3.data = 1 / dyn1.depth;
        gt_value.feature_4.data = dyn1.depth;
